...
class BookInfo(models.Model):
    """图书模型类"""
    title = models.CharField(verbose_name="书名", max_length=20, unique=True, db_index=True, db_column='title')
...
class AreaInfo(models.Model):
    """地区模型类"""
    title = models.CharField(verbose_name="地区", max_length=20)  # 名称
    # 自关联：地区、分类等表结构很相似且每个表的数据量很少,可以设计成一张表,内部关系字段指向本表主键 --> 是特殊的一对多模型
    # 关系属性使用self指向本类,由于一级数据没有父级,所以null和blank要允许为空
    parent = models.ForeignKey('self', verbose_name="上级地区", null=True, blank=True)  # 上级关系
